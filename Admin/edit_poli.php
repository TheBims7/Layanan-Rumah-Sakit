...
if (!isset($_GET['id'])) {
    echo "<script>alert('ID tidak ditemukan'); window.location.href='data_poli.php';</script>";
    exit();
}

$id = $_GET['id'];
$query = mysqli_query($conn, "SELECT * FROM poliklinik WHERE id = '$id'");
$data = mysqli_fetch_assoc($query);

if (!$data) {
    echo "<script>alert('Data tidak ditemukan'); window.location.href='data_poli.php';</script>";
    exit();
}
?>
...
<body class="bg-gray-100">
    ...
    <main class="content">
        ...
        <div class="box">
            <div class="form">
                <form action="proses_edit_poli.php" method="POST">
                    <input type="hidden" name="id" value="<?= $data['id'] ?>">
                    <h2>Nama Poliklinik</h2>
                    <input type="text" name="poli" value="<?= $data['poli'] ?>">
                    <h2>Gedung</h2>
                    <input type="text" name="gedung" value="<?= $data['gedung'] ?>"><br>
                    <button type="submit" name="edit_poli">Update Poliklinik</button>
                </form>
            </div>          
        </div>                 
    </main>
